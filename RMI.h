...
#ifndef POLYFIT_CLION_RMI_H
#define POLYFIT_CLION_RMI_H

#include <vector>
#include <stx/btree.h>
#include <stx/btree_map.h>

// as in linux, mlpack could be directly installed from apt and insert into sys path
...
public:
    ...
    StageModel(const arma::mat& dataset, const arma::rowvec& labels, vector<int> &architecture, int type = 0, int error_threshold = 100, double Trel = 0.01) {
        ...
        this->btree_replacement_count = btree_count;
    ...
    void InitQuerySet(const arma::mat& queryset) {
        ...
        int count = 0;
        arma::rowvec order(queryset.n_cols);
        arma::mat query;
    ...
    void Predict(const arma::mat& queryset, arma::rowvec& predictions) {
        ...
        predictions.clear();
        arma::mat predictions_with_order;
        ...
        for (int i = 0; i < stage_model.size(); i++) {
            for (int j = 0; j < stage_model[i].size(); j++) {
                ...
                if (i < stage_model.size() - 1) {
                    ...
                    predictions_temp.for_each([=](arma::mat::elem_type& val) {
                        val /= TOTAL_SIZE;
                    ...
                    for (int k = 0; k < stage_model[i + 1].size(); k++) {
                        arma::uvec indices = find(predictions_temp == k);
                    ...
                    arma::mat temp = join_cols(predictions_temp, stage_queryset[i][j].row(1)); // first row on top, second row on the bottom
                    predictions_with_order = join_rows(predictions_with_order, temp);
                }
            }
        }

        arma::uvec index = sort_index(predictions_with_order.row(1), "ascend");
    ...
    QueryResult CountPrediction(vector<Tk> &queryset_low, vector<Tk> &queryset_up, vector<Tv> &results, vector<Tk> &key_v, bool DoRefinement = true, string RealResultPath = "C:/Users/Cloud/iCloudDrive/LearnedAggregate/VLDB_Final_Experiments/RealQueryResults/TWEET_1D.csv"){
        ...
        query_result.tree_paras = this->btree_replacement_count;
    ...
    void TrainBucketAssigner(string filename) {
        ...
        arma::rowvec labels = dataset.row(1);
    ...
    int btree_replacement_count = 0;
    double Trel; // used for perform refinement
};

#endif //POLYFIT_CLION_RMI_H
