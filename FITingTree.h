...
#ifndef POLYFIT_CLION_FITINGTREE_H
#define POLYFIT_CLION_FITINGTREE_H
...
public:
    ...
    void TrainAtree(const arma::rowvec& dataset, const arma::rowvec& labels) {
        ...
        for (int i = 0; i < dataset_range.size(); i++) {
            bottom_layer_index.insert(pair<Tk, int>(dataset_range[i].first, i));
        }
    ...
    void TrainAtree(vector<Tk> &key_v, vector<Tv> &position_v) {
    ...
    void PredictExact(vector<Tk> &queryset, vector<Tv> &results) {
        typename stx::btree<Tk, int>::iterator iter;
        results.clear();
        int model_index = 0;
        double a, b;
        double result;
    ...
    QueryResult CountPrediction2(vector<Tk> &queryset_low, vector<Tk> &queryset_up, vector<Tv> &results, vector<Tk> &key_v, bool DoRefinement = true, string RealResultPath = "C:/Users/Cloud/iCloudDrive/LearnedAggregate/VLDB_Final_Experiments/RealQueryResults/TWEET_1D.csv") {

        // build the full key index
        stx::btree<Tk, int> full_key_index;
        for (int i = 0; i < key_v.size(); i++) {
            full_key_index.insert(pair<Tk, int>(key_v[i], i));
        }
        ...
        query_result.total_paras = this->dataset_range.size() * 3 + query_result.tree_paras;
    ...
    vector<Tk> keys;
    vector<pair<Tk, Tk>> dataset_range;
